...
def triple_barrier(df: pd.DataFrame, pct: float = 0.015, t_max: int = 5) -> pd.Series:
    """
    Adaptive triple-barrier: uses ATR-based width so barriers are always
    reachable. Falls back to simple up/down if barriers still not triggered.
    """
    ...
    # Compute realized vol to set adaptive barrier width
    log_r  = np.diff(np.log(prices + 1e-9))
    rv5    = np.array([log_r[max(0,i-5):i].std() if i>1 else pct for i in range(n)])
    rv5    = np.maximum(rv5, 0.002)   # min barrier = 0.2%

    for i in range(n - t_max):
        p0  = prices[i]
        # Adaptive: use max(pct, 1.5 * 5-bar realized vol)
        ...
        for j in range(1, t_max + 1):
            ...
            if p <= sl:
                lbl = -1
                break
        # If expired: label by direction of t_max-bar return
        if lbl == 0:
            ret_fwd = (prices[min(i + t_max, n-1)] / p0) - 1.0
            if   ret_fwd >  0.0005: lbl =  1
            elif ret_fwd < -0.0005: lbl = -1
            # else lbl = 0 (genuine flat)
        labels[i] = lbl
    ...
    def fit(self, X, y_tb, splits, verbose=True):
        ...
        for tr, te in splits:
            ...
            if len(np.unique(y_tr_fold)) < 2:
                continue
            gbm.fit(X[tr], y_tr_fold)
            oof_p[te] = gbm.predict_proba(X[te])[:, 1]
        # If OOF labels are single class, use simple direction from returns
        if len(np.unique(y_dir)) < 2:
            y_dir_balanced = np.zeros(len(y_dir))
            y_dir_balanced[len(y_dir)//2:] = 1  # force two classes
            gbm.fit(X, y_dir_balanced)
        else:
            gbm.fit(X, y_dir)
        self.primary = gbm
        ...
        for tr, te in splits:
            ...
            y_meta_fold = y_meta[tr]
            if len(np.unique(y_meta_fold)) < 2:
                continue
            et.fit(Xp_tr, y_meta_fold)
...
class EliteQuantEngine:
    ...
    def train(self, df: pd.DataFrame, fund: pd.DataFrame, verbose=True):
        ...
        y_dir_cpcv = (y_tb == 1).astype(int)
        ret_s = df["close"].pct_change().fillna(0)
        ret_s_loc = ret_s.loc[idx]
        if len(np.unique(y_dir_cpcv)) >= 2:
            self.cpcv_sh = cpcv_sharpe(oof_p, y_dir_cpcv, ret_s_loc)
        else:
            self.cpcv_sh = 0.0
