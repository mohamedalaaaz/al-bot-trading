...
class DirectionEngine:
    ...
    def score(self, df, fund, qm: QuantMath,
        ...
        cvd3=cvd20.diff(3)
        # Minimum CVD magnitude gate: require cvd3 to move at least 1 rolling std dev.
        # Without this, any tiny positive cvd3 (e.g. +0.001) fires the divergence.
        cvd3_std = float(cvd3.rolling(50).std().iloc[-1]) or 1.0
        cvd3_thr = max(cvd3_std * 0.8, 1.0)   # 0.8σ minimum, never below 1 unit
        div_b=bool(pr3.iloc[-1]<-0.12 and cvd3.iloc[-1] > cvd3_thr)
        div_s=bool(pr3.iloc[-1]> 0.12 and cvd3.iloc[-1] < -cvd3_thr)
        ...
        # Statistical gate: only fire if ou_z is significant (p < 0.05)
        # AND half-life is short enough to be tradeable (< 50 bars = 4h on 5m tf)
        ou_sig  = ou_res.get("significant", False) and ou_hl < 50
        ...
        if ou_sig and ou_z < -thr_ou:
            cats_bull.append("ou")
            pts_bull["ou"] = (3 if ou_z < -3 else 2) * bw * iw * ou_res.get("revert_conf", 1)
            active["ou"] = "+z={:.2f} p={:.3f} HL={:.1f}b regime={}".format(
                ou_z, ou_res["p_value"], ou_hl, vr["regime"])
        elif ou_sig and ou_z > thr_ou:
            cats_bear.append("ou")
            pts_bear["ou"] = (3 if ou_z > 3 else 2) * bw * iw * ou_res.get("revert_conf", 1)
            active["ou"] = "-z={:.2f} p={:.3f} HL={:.1f}b".format(
                ou_z, ou_res["p_value"], ou_hl)
...
def build_features(df, fund=None):
    ...
    # ou_z: rolling 50-bar z-score — proxy for OU equilibrium z-score without look-ahead.
    # The exact MLE value (ou_mle) is used live in DirectionEngine; here we need a
    # per-row value so the ML model has a real feature, not a constant zero.
    F["ou_z"] = (c_ - mu50) / sg50  # zero-mean, unit-variance per bar
    ...
    cvd3_std_feat = cvd3.rolling(50).std().replace(0, 1.0)
    cvd3_thr_feat = (cvd3_std_feat * 0.8).clip(lower=1.0)
    F["cvd_div_b"] = ((pr3 < -0.12) & (cvd3 >  cvd3_thr_feat)).astype(float)
    F["cvd_div_s"] = ((pr3 >  0.12) & (cvd3 < -cvd3_thr_feat)).astype(float)
    ...
    if fund is not None and len(fund)>=3: avg_fr=float(fund["fundingRate"].tail(8).mean())
    F["fund_rate"] = avg_fr
    # Funding trend: slope of last 8 rates, clipped to [-3, 3] (normalised units).
    # Rising positive funding = increasing long crowding = bearish mean-reversion signal.
    if fund is not None and len(fund) >= 4:
        rates_ = fund["fundingRate"].tail(8).values.astype(float)
        if len(rates_) >= 2:
            x_ = np.arange(len(rates_))
            fund_slope = float(np.polyfit(x_, rates_, 1)[0])
            F["fund_trend"] = float(np.clip(fund_slope * 1000, -3., 3.))
        else:
            F["fund_trend"] = 0.
    else:
        F["fund_trend"] = 0.
...
def market_profile(df, tick=25.):
    ...
    # Reset to positional index so +1/-1 arithmetic works correctly
    pf  = pd.DataFrame({"p": list(vm.keys()), "v": list(vm.values())}) \
            .sort_values("p").reset_index(drop=True)
    tot = pf["v"].sum()
    pi  = int(pf["v"].idxmax())   # positional index of POC
    poc = float(pf.loc[pi, "p"])
    lo_i = pi; hi_i = pi
    cum  = float(pf.loc[pi, "v"])
    while cum / tot < 0.70:
        can_up = hi_i + 1 < len(pf)
        can_dn = lo_i - 1 >= 0
        if not can_up and not can_dn: break
        up_v = float(pf.loc[hi_i+1, "v"]) if can_up else 0.
        dn_v = float(pf.loc[lo_i-1, "v"]) if can_dn else 0.
        if up_v >= dn_v and can_up:
            ...
            break
    vah  = float(pf.loc[hi_i, "p"])
    val_ = float(pf.loc[lo_i, "p"])
    return poc, vah, val_
...
class ModelStore:
    def __init__(self,d):
        ...
        self.lat=os.path.join(d,"latest.pkl"); self.best_=os.path.join(d, "../../Downloads/best.pkl")
...
def display(price,dr,ml,final,bars,live,paper_st,sh,ws_conn,ckpt,
    ...
    print(f"  {cc(f'${price:,.2f}','W')}  GARCH: {cc(g['regime'],vc)} pct={g['pct']:.0f} α+β={g['persistence']:.4f}")
    print(f"  EVT: ξ={e.get('xi',0):.3f} CVaR99={e.get('cvar_99',0):.4f} tail={e.get('tail_class','?').split('/')[0]}")
    lc="R" if l["recent_jump"] else "D"
    print(f"  Lévy: jumps={l['n_jumps']} λ={l['lambda']:.4f} recent={cc(str(l['recent_jump']),lc)} penalty×{l['size_penalty']:.1f}")
    print(f"  VR-Regime: {cc(vr_r.get('regime','?'),'M')}  SPRT LLR_bull={sprt_st['llr_bull']:+.3f} bear={sprt_st['llr_bear']:+.3f} n={sprt_st['n']}")
    print()
    ...
    if side=="BUY":   print(cc(f"  ||  ####  B U Y  ^^  #### cats={dr['n_bull']} score={sc:+.1f} size×{final['garch_m']:.2f}  ||","G"))
        ...
        why=("no signals" if nb==0 and nbe==0 else
             ...
             f"ML blocked P={ml.get('prob',0.5):.3f}" if not ml.get("allow") else
             f"score={sc:+.1f}")
        print(cc(f"  ||  ----  W A I T  ({why})  ||","Y"))
    ml_c="G" if ml.get("allow") else "R"
    print(f"  ||  Score:{cc(f'{sc:>+.1f}','B')} {cc(b,sc_c)} Conf:{cc(f'{conf:.0f}%','B')} ML:{cc(f'{ml.get('prob',0.5):.3f}',ml_c)}({cc('PASS','G') if ml.get('allow') else cc('BLOCK','R')}) Youden_t={ml.get('youden_t',0.5):.3f} ||")
    print(cc("  "+"="*66,"W"))
    print()
    ...
    if final.get("tradeable") and final.get("tp1"):
        ...
        print(cc("  +--- TRADE ---------------------------------------------------------+","Y"))
        print(f"  |  Entry:  ${price:>12,.2f}  ATR-mult={final.get('atr_mult',1.5):.2f}×{'':29}|")
        ...
        print(f"  |  R:R={cc(f'{rr:.2f}x',rc)} Qty={final['qty']:.4f}BTC Bayesian-Kelly={final['kelly']*100:.2f}%{'':16}|")
        print(cc("  +-------------------------------------------------------------------+","Y"))
    elif side!="WAIT":
        print(cc(f"  conf={conf:.0f}%  R:R={final.get('rr',0):.2f}x  not tradeable","Y"))
    print()
    ...
    print(f"  VWAP    {dr['vwap_dev']:>+.3f}σ  Wyckoff={dr['wyckoff']}  FR={dr['avg_fr']*100:.5f}%")
    print()
    ...
    for sig,sd in list(bsumm.items())[:6]:
        ...
        print(f"  {sig:<12} P={cc(f'{pm:.3f}',col)} CI=[{sd['ci_lo']:.3f},{sd['ci_hi']:.3f}] BF={bf:.1f}{bf_s} IC={ic:>+.4f} w={w:.2f}× n={n_}")
    print()

    print(cc("  -- KELLY SIZING -------------------------------------------------------","D"))
    print(f"  Bayesian-Kelly={final['kelly']*100:.2f}%  GARCH×{final['garch_m']:.2f}  CVaR-mult×{final.get('cvar_m',1.0):.2f}  Kurt-adj×{CFG.get('KURT_KELLY',1.0):.3f}")
    print(f"  Jump-penalty×{l['size_penalty']:.1f}  Final-size×{final['size_multiplier']:.3f}")
    print()

    if paper_st:
        pc="G" if paper_st["pnl_pct"]>=0 else "R"
        print(cc("  -- PAPER TRADING ------------------------------------------------------","M"))
        print(f"  Balance:{cc(f'${paper_st['balance']:,.2f}','W')}  PnL:{cc(f'{paper_st['pnl_pct']:+.2f}%',pc)}  WR={paper_st['wr']:.1f}%  Trades={paper_st['trades']}  {cc('IN POS','G') if paper_st['in_pos'] else ''}")
        print()
    ...
    if recent:
        ...
        for s in reversed(recent):
            ...
            print(f"  {s['time'].strftime('%H:%M:%S')} {s['side']:>4}  {s['score']:+.1f}pts  conf={s['conf']:.0f}%  cats={s['n_ind']}  {cc(oc,occ)}")
        print()
...
class EliteV8:
    def __init__(self,account=1000.,paper=True,model_dir=CFG["MODEL_DIR"]):
        ...
        self._ic_buf   = {}   # signal values buffered for next-bar IC recording
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # IC tracker: record signal values THIS bar; they get matched to the
                # NEXT bar's return (one bar later), which is the correct forward IC.
                # We buffer the signals here and flush them on the following bar.
                if self.bars > 2 and hasattr(self, "_ic_buf") and self._ic_buf:
                    fut_ret = float(curr_df["close"].pct_change().iloc[-1])
                    for sig_name_, sig_val_ in self._ic_buf.items():
                        self.qm.record_ic(sig_name_, sig_val_, fut_ret)
                # Capture current-bar signal values for next iteration.
                # Sign convention: record the signal in its BUY-predicting direction
                # so that positive signal → positive future return → positive IC.
                # OU:     positive ou_z = overbought = BEARISH → negate so +means oversold
                # Kalman: positive trend = uptrend = BULLISH   → keep sign
                # CVD:    positive delta = net buying = BULLISH → keep sign
                ou_now  = self.qm.stoch.ou_mle(curr_df["close"].values[-50:])
                kal_now = self.qm.stoch.rts_kalman(curr_df["close"].values[-50:])
                self._ic_buf = {
                    "ou":     -float(ou_now["ou_z"]),          # negated: +means oversold
                    "cvd":     float(curr_df["delta_pct"].iloc[-1]),
                    "kalman":  float(kal_now["live_trend"]),
                }
                ...
                # Reset only after CONFIRM (evidence acted upon) — not on REJECT,
                # which just means evidence is insufficient so far, not contradictory.
                sprt_d = self.qm.sprt.update(ml.get("prob", 0.5))
                if sprt_d in ["CONFIRM_BUY", "CONFIRM_SELL"]:
                    self.qm.sprt.reset()
                # REJECT means ML evidence is actively contradictory — block the trade
                sprt_blocked = (sprt_d == "REJECT")
                ...
                # Confidence
                # Confidence = Bayesian posterior P(win | active categories).
                # p_win per category comes from Beta(alpha, beta) posterior updated
                # after each resolved trade. Blended with ML probability weighted by
                # how much data we have: few obs → trust ML more; many → trust Bayes.
                active_cats = dr["cats_bull"] if side == "BUY" else dr["cats_bear"]
                if active_cats:
                    p_bayes_list = [self.qm.bayes.p_win(c) for c in active_cats]
                    ic_wts       = np.array([max(self.qm.information_coefficient(c)+1, 0.2)
                                             for c in active_cats])
                    ic_wts       = ic_wts / (ic_wts.sum() + 1e-9)
                    p_bayes      = float(np.dot(p_bayes_list, ic_wts))
                    n_obs_min    = min(self.qm.bayes._n.get(c, 0) for c in active_cats)
                    data_w       = float(np.clip(n_obs_min / 20.0, 0.0, 1.0))
                    p_ml         = float(ml.get("prob", 0.5))
                    if side == "SELL": p_ml = 1.0 - p_ml
                    conf = float(np.clip((data_w * p_bayes + (1.0 - data_w) * p_ml) * 100, 0, 99))
                else:
                    conf = 50.0

                tradeable = (side != "WAIT" and rr >= CFG["MIN_RR"]
                             and conf >= 40.0 and not sprt_blocked)
