...
def display(price,dr,ml,final,bars,live,paper_st,sh,ws_conn,ckpt,
    ...
    _sc_s  = cc("{:>+.1f}".format(sc), "B")
    _cf_s  = cc("{:.0f}%".format(conf), "B")
    _ml_s  = cc("{:.3f}".format(ml.get("prob", 0.5)), ml_c)
    _gt_s  = cc("PASS","G") if ml.get("allow") else cc("BLOCK","R")
    _yd    = ml.get("youden_t", 0.5)
    print("  ||  Score:{} {} Conf:{} ML:{}({}) Youden_t={:.3f} ||".format(
        _sc_s, cc(b, sc_c), _cf_s, _ml_s, _gt_s, _yd))
    print(cc("  "+"="*66,"W"))
    print()
    ...
    if paper_st:
        ...
        _bal  = cc("${:,.2f}".format(paper_st["balance"]), "W")
        _pnl  = cc("{:+.2f}%".format(paper_st["pnl_pct"]), pc)
        _inpos = cc("IN POS","G") if paper_st["in_pos"] else ""
        print("  Balance:{}  PnL:{}  WR={:.1f}%  Trades={}  {}".format(
            _bal, _pnl, paper_st["wr"], paper_st["trades"], _inpos))
        print()
