...
def build_features(df: pd.DataFrame, fund: pd.DataFrame = None) -> pd.DataFrame:
    ...
    # OU z-score — rolling approximation (avoids look-ahead bias in ML training).
    # Full ou_mle() is used in DirectionEngine.score() for the live bar only.
    # Here we use the rolling 50-bar z-score as the OU feature: mathematically
    # equivalent to the OU equilibrium z when κ is estimated from recent history.
    # This gives a different value per row (no look-ahead), unlike the old code
    # which called ou_mle(c_.values[-100:]) and broadcast one scalar to all rows.
    F["ou_z"] = (c_ - mu50) / sg50   # same as z_50 but kept separate for clarity
...
class DirectionEngine:
    ...
    def score(self, df: pd.DataFrame, fund: pd.DataFrame = None,
        ...
        # ── 2. OU MEAN REVERSION — exact MLE (Shoji-Ozaki 1998), not OLS ──
        # OLS Δx=a+bx is biased (non-exogenous regressor). MLE is consistent.
        # Same ou_mle() used here AND in build_features() — no more dual estimates.
        x_ou  = c_.values[-100:] if len(c_) >= 100 else c_.values
        _ou   = ou_mle(x_ou)
        ou_z  = _ou["ou_z"]
        # Explicit p-value under H0: ou_z ~ N(0,1). Two-tailed.
        ou_pval = float(2.0 * stats.norm.sf(abs(ou_z)))
        thr_ou  = CFG["OU_Z_THRESH"]
        # Gate: z beyond threshold AND statistically significant AND process is reverting fast
        ou_valid = _ou["valid"] and ou_pval < 0.05 and _ou["half_life"] < 50
        if ou_valid and ou_z < -thr_ou:
            pts = 3 if ou_z < -3.0 else 2
            cats_bull.append("ou"); pts_bull["ou"] = pts
            active["ou"] = "+z={:.2f} p={:.3f} hl={:.1f}bar OVERSOLD".format(
                ou_z, ou_pval, _ou["half_life"])
        elif ou_valid and ou_z > thr_ou:
            pts = 3 if ou_z > 3.0 else 2
            cats_bear.append("ou"); pts_bear["ou"] = pts
            active["ou"] = "-z={:.2f} p={:.3f} hl={:.1f}bar OVERBOUGHT".format(
                ou_z, ou_pval, _ou["half_life"])
...
def garch11(ret):
    ...
    # Continuous inverse-volatility sizing: σ_median / σ_current
    # When vol is 2× the median → size 0.5×. When vol is 0.5× median → size 1.5×.
    # Capped at [0.25, 2.0] to prevent extreme leverage or zero sizing.
    vol_series = np.sqrt(h)
    med_vol    = float(np.median(vol_series[vol_series > 0]))
    sm = float(np.clip(med_vol / max(cv, 1e-10), 0.25, 2.0))
    return cv, sm, rg, vp
...
def market_profile(df, tick=25.0):
    ...
    pf = pd.DataFrame({"p":list(vm.keys()),"v":list(vm.values())}).sort_values("p").reset_index(drop=True)
    tot = pf["v"].sum()
    poc_pos = int(pf["v"].idxmax())   # positional index of POC row
    poc     = float(pf.loc[poc_pos,"p"])

    # Value area: expand outward from POC using positional indices (not label ops)
    # At each step, add whichever adjacent row has more volume.
    lo_pos = poc_pos; hi_pos = poc_pos
    cum    = float(pf.loc[poc_pos,"v"])
    while cum / tot < 0.70:
        can_up = hi_pos + 1 < len(pf)
        can_dn = lo_pos - 1 >= 0
        if not can_up and not can_dn: break
        up_v = float(pf.loc[hi_pos+1,"v"]) if can_up else 0.0
        dn_v = float(pf.loc[lo_pos-1,"v"]) if can_dn else 0.0
        if up_v >= dn_v and can_up:
            hi_pos += 1; cum += up_v
        elif can_dn:
            lo_pos -= 1; cum += dn_v
        else:
            break
    vah = float(pf.loc[hi_pos,"p"])
    val = float(pf.loc[lo_pos,"p"])
    return poc, vah, val
...
class ModelStore:
    def __init__(self, d=CFG["MODEL_DIR"]):
        ...
        self.lat=os.path.join(d,"latest.pkl"); self.best=os.path.join(d, "../../Downloads/best.pkl")
...
class PaperTrader:
    def __init__(self,acc):
        ...
        self.last_cats = []   # signal categories active when last trade opened
    @property
    ...
    def enter(self,side,entry,sl,tp1,tp2,qty,score,conf,reason,cats=None):
        with self.lock:
...
class SigHistory:
    def __init__(self,mx=200):
        ...
        self._pnl_log = deque(maxlen=100)   # trade returns for running Sharpe
        ...
        with self.lock:
            for s in reversed(self.sigs):
                if s["out"] is None and s["side"]!="WAIT":
                    ...
                    if ok: self.ok+=1
                    # Record fractional return for Sharpe
                    ret = (fp - s["price"]) / s["price"] * (1 if s["side"]=="BUY" else -1)
                    self._pnl_log.append(float(ret))
                    break
    ...
    def sharpe(self) -> float:
        """Annualised Sharpe on resolved signal returns (not trade-level, bar-level proxy)."""
        if len(self._pnl_log) < 5: return 0.0
        r = np.array(self._pnl_log)
        return float(np.mean(r) / (np.std(r) + 1e-10) * math.sqrt(252*24*12))  # 5m bars

    def win_rate_ci(self) -> tuple:
        """
        90% Bayesian credible interval on win rate.
        Posterior: Beta(alpha_0 + wins, beta_0 + losses) where alpha_0=beta_0=1 (Jeffreys prior).
        """
        wins   = self.ok;  losses = self.tot - wins
        a = 1.0 + wins;    b = 1.0 + losses
        lo = float(stats.beta.ppf(0.05, a, b))
        hi = float(stats.beta.ppf(0.95, a, b))
        return lo, hi
...
def display(price, dr, ml, final, tr, loop_n, live,
    ...
    # Show p-value explicitly for OU signal
    ou_pval = float(2.0 * stats.norm.sf(abs(ou)))
    print("  OU z={:>+.3f}  p={:.3f}  {}  (threshold ±{}, p<0.05)".format(
        ou, ou_pval,
    ...
    # ── New: Kelly, SPRT, Bayesian posteriors ─────────────────────────
    sprt_str  = final.get("sprt","?")
    sprt_col  = "G" if "CONFIRM" in sprt_str else ("R" if "REJECT" in sprt_str else "Y")
    kelly_f   = final.get("kelly_f", 0.0)
    p_kelly   = final.get("p_kelly", 0.5)
    print("  SPRT:{}  Kelly f*={:.4f} ({:.1f}%)  P_win_kelly={:.3f}".format(
        cc(sprt_str, sprt_col), kelly_f, kelly_f*100, p_kelly))
    print()
    ...
    recent=sh.recent(6)
    sharpe    = sh.sharpe()
    wr_lo, wr_hi = sh.win_rate_ci()
    sh_col    = "G" if sharpe > 0.5 else ("R" if sharpe < 0 else "Y")
    if recent:
        print(cc("  -- SIGNAL HISTORY  acc:{:.1f}%  CI:[{:.0f}%,{:.0f}%]  Sharpe:{:.2f}  (n={}) ---".format(
            sh.acc, wr_lo*100, wr_hi*100, sharpe, sh.tot),"D"))
    ...
    # ── Bayesian posteriors per signal category ──────────────────────────
    if bayes and bayes.n_obs:
        print(cc("  -- BAYESIAN POSTERIORS (Beta conjugate, Jeffreys prior) -----------","D"))
        all_cats = ["cvd","ou","kalman","wyckoff","vwap","sweep","funding","trap","tick"]
        for cat in all_cats:
            n_ = bayes.n_obs.get(cat, 0)
            if n_ == 0: continue
            pw   = bayes.p_win(cat)
            bfw  = bayes.bf_weight(cat)
            ic_v = ic.ic(cat) if ic else 0.0
            # 90% credible interval
            a_ = bayes.alpha_.get(cat, 2.0); b_ = bayes.beta_.get(cat, 2.0)
            ci_lo = float(stats.beta.ppf(0.05, a_, b_))
            ci_hi = float(stats.beta.ppf(0.95, a_, b_))
            pw_col = "G" if pw > 0.55 else ("R" if pw < 0.45 else "D")
            print("  {:10s}  p_win={}  CI:[{:.2f},{:.2f}]  BF×{:.2f}  IC={:+.3f}  n={}".format(
                cat, cc("{:.3f}".format(pw), pw_col),
                ci_lo, ci_hi, bfw, ic_v, n_))
        print()
...
class EliteV71:
    def __init__(self, account=1000., paper=True, reset=False):
        ...
        # ── Statistical components (wired, not dead code) ──────────────
        # SPRT: sequential test accumulating P(win) evidence bar by bar.
        # Resets after CONFIRM or REJECT decision is acted upon.
        self.sprt      = SPRT(p0=CFG["SPRT_H0"], p1=CFG["SPRT_H1"],
                              alpha=CFG["SPRT_ALPHA"], beta=CFG["SPRT_BETA"])
        self._sprt_reset_in = 0   # bars until forced reset (prevent infinite accumulation)

        # BayesianTracker: Beta(alpha, beta) posterior per signal category.
        # Updated after each resolved trade. p_win(cat) drives Kelly sizing.
        self.bayes     = BayesianTracker()

        # ICTracker: Spearman IC between signal value and next-bar return.
        # Weights each signal category by its realized predictive power.
        self.ic        = ICTracker(window=30)
        self._last_sig_vals = {}   # {cat: signal_value} for IC recording
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # ── SPRT: accumulate ML evidence bar by bar ──────────────
                # Feed each bar's ML probability into the sequential ratio test.
                # This prevents acting on a single noisy bar — waits for statistical
                # evidence to accumulate to the Wald boundaries.
                ml_p = ml.get("ml_prob", 0.5)
                self.sprt.update(ml_p)
                sprt_dec = self.sprt.decision()

                # Force reset every 20 bars so SPRT doesn't wait forever
                self._sprt_reset_in += 1
                if self._sprt_reset_in > 20:
                    self.sprt.reset(); self._sprt_reset_in = 0

                # ── IC tracking: record signal values for next resolution ─
                if dr["side"] != "WAIT":
                    self._last_sig_vals = {
                        "ou":      dr.get("ou_z", 0.0),
                        "kalman":  dr.get("kal_trend", 0.0),
                        "vwap":    dr.get("vwap_dev", 0.0),
                        "wyckoff": float(dr.get("wyckoff", 0)),
                    }

                # ── Confidence — Bayesian posterior, not n_ind/5 hack ────
                # p_win per category from Beta posterior (prior: Beta(2,2) = uniform-ish).
                # Combined with ML probability via evidence-weighted blend.
                # If n_obs < 5 per category, posterior stays near 0.5 (uninformative).
                active_cats = dr["cats_bull"] if dr["side"]=="BUY" else dr["cats_bear"]
                if active_cats:
                    # Per-category Bayesian posterior win probability
                    p_bayes = float(np.mean([self.bayes.p_win(c) for c in active_cats]))
                    # IC-weighted average: high-IC signals pull posterior toward their direction
                    ic_wts  = np.array([self.ic.weight(c) for c in active_cats])
                    p_ic    = float(np.average([self.bayes.p_win(c) for c in active_cats],
                                               weights=ic_wts))
                    # Data weight: trust Bayes more as sample size grows
                    n_obs_min = min(self.bayes.n_obs.get(c, 0) for c in active_cats)
                    data_w    = float(np.clip(n_obs_min / 20.0, 0.0, 1.0))
                    # Blend: low data → rely on ML; high data → rely on Bayes+IC
                    p_ml  = float(ml_p if dr["side"]=="BUY" else 1.0 - ml_p)
                    p_combined = data_w * p_ic + (1.0 - data_w) * p_ml
                    conf  = float(np.clip(p_combined * 100, 0, 99))
                else:
                    conf  = 50.0

                # ── Kelly Criterion position sizing ──────────────────────
                # f* = (p*b - q) / b  where b=R:R, p=win_prob, q=1-p
                # Use half-Kelly for robustness. Cap at MAX_RISK.
                rr_est = rr if (tp1 and sl_ and price != sl_) else CFG["MIN_RR"]
                # Win probability from Bayesian posterior (falls back to ML if no history)
                if active_cats and min(self.bayes.n_obs.get(c, 0) for c in active_cats) >= 5:
                    p_kelly = float(np.mean([self.bayes.p_win(c) for c in active_cats]))
                else:
                    p_kelly = float(np.clip(ml_p if dr["side"]=="BUY" else 1-ml_p, 0.1, 0.9))
                f_full  = max(0.0, (p_kelly * rr_est - (1.0 - p_kelly)) / max(rr_est, 0.01))
                f_half  = float(np.clip(f_full * 0.5, 0.0, CFG["MAX_RISK"]))  # half-Kelly
                # Multiply by GARCH multiplier (volatility regime adjustment)
                qty = (CFG["ACCOUNT"] * f_half * garch_m / max(stop_dist, 1.)) if sl_ else 0.

                n_ind = dr.get("n_indep", 0)

                tradeable = (side!="WAIT" and rr>=CFG["MIN_RR"] and conf>=CFG["MIN_CONF_PCT"])

                # Additional SPRT gate: only trade if SPRT has confirmed direction
                # (or if we have insufficient history to use SPRT — first 10 bars)
                sprt_ok = (sprt_dec["decision"] in ("CONFIRM_BUY","CONFIRM_SELL","CONTINUE")
                           or self.bars < 10)
                if tradeable and not sprt_ok:
                    tradeable = False

                final = {"side":side,"tradeable":tradeable,"sl":sl_,"tp1":tp1,"tp2":tp2,
                         "qty":round(qty,3),"rr":rr,"confidence":conf,
                         "kelly_f":round(f_half, 4), "p_kelly":round(p_kelly, 3),
                         "sprt":sprt_dec["decision"]}

                if self.bars > 6: self.sh.resolve(price)

                # ── Bayesian update after trade outcome ──────────────────
                # Update per-category win/loss posterior once a position closes.
                if self.paper:
                    res = self.paper.update(price)
                    if res and res["type"] in ("WIN","LOSS","TP1"):
                        won = res["type"] in ("WIN","TP1")
                        # Update each active category's posterior
                        for cat in self.paper.last_cats:
                            self.bayes.update(cat, won)
                        if res["type"] != "TP1":
                            self.sprt.reset(); self._sprt_reset_in = 0
                        print(cc("  [PAPER] {}  ${:+.2f}  @${:,.2f}".format(
                            res["type"],res["pnl"],res["price"]),"G" if won else "R"))

                # ── IC recording: actual next-bar return for open signals ─
                # Record signal value → realized return for Spearman IC tracking.
                if hasattr(self, "_last_sig_vals") and self._last_sig_vals:
                    ret_1bar = float(curr_df["close"].pct_change().iloc[-1])
                    for cat, val_sig in self._last_sig_vals.items():
                        self.ic.record(cat, val_sig, ret_1bar)
                    self._last_sig_vals = {}

                if self.paper and tradeable and not self.paper.position:
                    reason = " | ".join(str(v) for v in dr.get("active",{}).values())
                    cats_active = dr["cats_bull"] if side=="BUY" else dr["cats_bear"]
                    if self.paper.enter(side,price,sl_,tp1,tp2,qty,
                                        dr["score"],conf,reason,cats=cats_active):
